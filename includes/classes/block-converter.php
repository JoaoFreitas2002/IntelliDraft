...
class BlockConverter
{
	...
	private function convert_to_blocks()
	{
		...
		foreach ($this->dom_content->getElementsByTagName('body')->item(0)->childNodes as $element) {
			...
			$has_tag_mapping = isset($this->mapping[$tag]);

			// Special handling for <p> containing only an <img>
			if ($tag === 'p' && $element->childNodes->length === 1 && $element->firstChild->nodeName === 'img') {
				$img_node = $element->firstChild;
				$this->block_content .= $this->serialize_block($this->image($img_node));
				$this->block_content .= "\n\n";
				continue;
			}

			// Handle standalone <img> directly
			if ($tag === 'img') {
				$this->block_content .= $this->serialize_block($this->image($element));
				$this->block_content .= "\n\n";
				continue;
			}

			// Original logic for other tags
			if ($has_tag_mapping && !$this->is_callback($this->mapping[$tag])) {
				$this->block_content .= $this->serialize_block(
					[
						'blockName' => $this->mapping[$tag],
						'attrs'     => [],
						'innerHTML' => $this->get_html($element),
					]
				);
				$this->block_content .= "\n\n";
			} elseif ($has_tag_mapping && $this->is_callback($this->mapping[$tag])) {
				$this->block_content .= $this->serialize_block(call_user_func($this->mapping[$tag], $element));
				$this->block_content .= "\n\n";
